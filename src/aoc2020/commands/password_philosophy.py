...
def part_two_is_valid_password(passwd: Password) -> bool:
    passwd.password_value.count(passwd.letter)
    low_letter: Optional[str]
    high_letter: Optional[str]

    try:
        low_letter = passwd.password_value[passwd.letter_low_limit - 1]
    except IndexError:
        low_letter = None

    try:
        high_letter = passwd.password_value[passwd.letter_high_limit - 1]
    except IndexError:
        high_letter = None
    ...
    if low_letter == passwd.letter or high_letter == passwd.letter:
        return True
...
def part_two_example() -> None:
    passwd_list = """1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc"""

    valid_passwords = 0
    for p in parse_password_input(passwd_list):
        if part_two_is_valid_password(p):
            valid_passwords += 1

    print("Answer: {}".format(valid_passwords))


@password_philosophy.command()
def part_two_actual() -> None:
    passwd_list = """6-10 p: ctpppjmdpppppp
...
def part_one_is_valid_password(passwd: Password) -> bool:
    letter_count = passwd.password_value.count(passwd.letter)

    if (
        letter_count >= passwd.letter_low_limit
        and letter_count <= passwd.letter_high_limit
    ):
        return True
...
2-9 c: ccccccccc"""
    ...
    valid_passwords = 0
    for p in parse_password_input(passwd_list):
        if part_one_is_valid_password(p):
            valid_passwords += 1
